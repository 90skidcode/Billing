...
function DashboardCard01(props) {
  ...
  useEffect(() => {
    let data = { "list_key": "GetDashCard", "sum_by": "price", "type": "sum", "range": "Daywise", "branch": JSON.parse(sessionStorage.getItem('details'))[0].pos_user_branch, "product": "" };
  ...
  const filterby = (type) => {
    let data = { "list_key": "GetDashCard", "sum_by": "price", "type": "sum", "range": "Daywise", "branch": type, "product": "" };
    PostApi(data).then((res) => {
      let tableresponce = res.responcePostData.data.result[0].bill_result;
      setCount(tableresponce);
  ...
  return (
    <div className="flex flex-col col-span-full sm:col-span-6 xl:col-span-3 bg-white shadow-lg rounded-sm border border-slate-200">
      <div className="px-5 pt-5">
        ...
        <h2 className="text-lg font-semibold text-slate-800 mb-2">Daily</h2>
