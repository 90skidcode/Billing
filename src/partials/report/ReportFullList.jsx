...
console.log('====================================');
console.log((JSON.parse(sessionStorage.getItem('details'))[0].pos_user_type));
console.log('====================================');
useEffect(() => {
              ...
              <div className="grow">
                <div className="grow items-center border-b border-slate-100 text-xs p-2 grid grid-cols-12 gap-2 ">
                  {BillReport.map((element) => (
                    <NavLink end to={`/report/${element.link}`} className="py-1 m-1 md:h-28 col-span-full md:col-span-3 border col-2 border-slate-400 bg-white border-l-4 cursor-pointer p-2 focus-visible:border-teal-900 focus-visible:outline-none  focus-visible:bg-teal-500 focus-visible:text-white"
                      tabIndex="0"
                      key={Math.random()}
                    >
